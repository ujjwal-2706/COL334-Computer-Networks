...
def generate_file(client_id,final_string):
    file_name = "client" + str(client_id) + ".txt"
    file = open(file_name,'w')
    file.write(final_string)
    file.close()
def checkComplete(client_id,local_data_client):
    chunk_id = 0
    result = []
    ...
    value = hashlib.md5(final_string.encode('utf-8')).hexdigest()
    if value== "9f9d1c257fe1733f6095a8336372616e":
        print(value)
        generate_file(client_id,final_string)
        return True
    else:
        return False
lock = threading.Lock()
def thread_function_udp(thread_number):
    ...
    while True:
        ...
        if chunk_id in local_data_file:
            chunk_id += 1
        if chunk_id >= total_size-5 and checkComplete(thread_number,local_data_file):
            break
    with lock:
        COMPLETE += 1
